  ...
  const menu =
    open && mounted ? (
      <div
        ref={menuRef}
        role="menu"
        className="fixed z-[100] max-h-[min(420px,70vh)] min-w-[220px] overflow-y-auto rounded-md border border-white/[0.08] bg-[#2a2a2a] py-0.5 shadow-lg thin-scroll"
        style={{ left: position.left, top: position.top }}
      >
        {FRAME_PRESET_CATEGORIES.map((cat) => (
          <div key={cat.id}>
            <div className="px-2.5 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-[#5c5c5c]">
              {cat.label}
            </div>
            {FRAME_PRESETS.filter((p) => p.category === cat.id).map((p) => (
              <button
                ...
                className={cn(
                  ...
                  framePresetId === p.id ? "bg-accent/20 text-white" : "text-[#d4d4d4] hover:bg-white/[0.06]",
                )}
        ...
        <button
          ...
          className={cn(
            "flex w-full items-center gap-2 px-2.5 py-1.5 text-left text-[11px] font-medium transition-colors",
            isCustom ? "bg-accent/20 text-white" : "text-[#d4d4d4] hover:bg-white/[0.06]",
          )}
        ...
        >
          ...
          <span className="shrink-0 text-[10px] text-[#6b6b6b]">drag</span>
        </button>
        <p className="px-2.5 pb-1.5 pt-0.5 text-[10px] leading-snug text-[#5c5c5c]">
