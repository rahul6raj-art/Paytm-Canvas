...
/** Above this, parsing/applying localStorage on the main thread can freeze the tab. */
export const MAX_LOCAL_DOCUMENT_BYTES = 4 * 1024 * 1024;
...
export type DocumentToEditorPatchOptions = {
  /** Skip hierarchy repair (caller already ran light import repair). */
  skipHierarchyRepair?: boolean;
};

export function documentToEditorPatch(
  doc: PaytmCraftDocument,
  opts?: DocumentToEditorPatchOptions,
): EditorPersistSlice {
  const base = {
    assets: doc.assets ?? {},
...
export function countDocumentNodes(doc: PaytmCraftDocument): number {
  if (doc.pages?.length) {
    let total = 0;
    for (const page of doc.pages) {
      total += Object.keys(page.nodes).length;
    }
    return total;
  }
...
export function isOversizedLocalDocumentRaw(raw: string): boolean {
  return raw.length > MAX_LOCAL_DOCUMENT_BYTES;
}

export function isOversizedLocalDocument(doc: PaytmCraftDocument): boolean {
  return countDocumentNodes(doc) > MAX_LOCAL_DOCUMENT_NODES;
}
...
export function readLocalDocument(): PaytmCraftDocument | null {
  ...
  try {
    ...
    if (!raw) return null;
    if (isOversizedLocalDocumentRaw(raw)) {
      console.warn(
        "[Paytm Craft] Local document is too large to load safely; clearing browser storage.",
      );
      clearLocalDocument();
      return null;
    }
    const doc = parsePaytmCraftDocumentJson(raw);
    if (doc && isOversizedLocalDocument(doc)) {
      console.warn(
        "[Paytm Craft] Local document has too many layers to load safely; clearing browser storage.",
      );
      clearLocalDocument();
      return null;
    }
...
export function writeLocalDocument(doc: PaytmCraftDocument): boolean {
  if (typeof window === "undefined") return false;
  const json = JSON.stringify(doc);
  if (isOversizedLocalDocumentRaw(json)) {
    console.warn(
      "[Paytm Craft] Document is too large to save in browser storage; keeping edits in memory only.",
    );
    return false;
  }
  try {
