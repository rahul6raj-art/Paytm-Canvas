...
function EditorLoadError({ message }: { message: string }) {
  return (
    <div className="flex h-dvh flex-col items-center justify-center gap-4 bg-app-bg px-6 text-center text-app-fg">
      <h1 className="text-base font-semibold">Editor failed to load</h1>
      <p className="max-w-md text-ui-sm leading-relaxed text-app-muted">
        The editor bundle could not be loaded. This is usually fixed by clearing the Next.js cache and
        restarting the dev server.
      </p>
      <pre className="max-w-lg overflow-x-auto rounded-lg border border-app-border bg-app-inset px-3 py-2 text-left text-caption text-app-subtle">
        rm -rf .next && npm run dev
      </pre>
      <p className="max-w-md text-caption text-app-subtle">{message}</p>
    </div>
  );
}
...
const AppShell = dynamic(
      ...
      }),
  {
    ssr: false,
    loading: () => (
      <div className="flex h-dvh items-center justify-center bg-chrome font-sans text-ui-sm text-app-muted">
        Loading editor…
      </div>
    ),
  },
